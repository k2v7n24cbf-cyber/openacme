...
const PEER_ASK_ERROR_MAX_CHARS = 4096;
...
function peerAskMetadata(
  m: unknown,
): { fromAgentId?: string; toAgentId?: string } | null {
  const meta = (m as { metadata?: MessageMetadata } | null)?.metadata;
  if (!meta || meta.kind !== "agent_chat") return null;
  return {
    fromAgentId:
      typeof meta["fromAgentId"] === "string"
        ? (meta["fromAgentId"] as string)
        : undefined,
    toAgentId:
      typeof meta["toAgentId"] === "string"
        ? (meta["toAgentId"] as string)
        : undefined,
  };
}
...
export class AgentManager {
  ...
  constructor(
    ...
    bindAgentTool({
      listAgents: () =>
        this.agentStore.list().map((def) => ({
          ...
          name: def.name,
          role: def.role ?? "",
          instantMessagesEnabled: def.instantMessagesEnabled ?? true,
        })),
    ...
    bindAgentAsk({
      ask: (request) => this.askAgent(request),
    });
  ...
  async updateAgent(
    ...
    const persisted = this.agentStore.get(id) ?? updated;
  ...
  private ownsPeerAskSession(
    ...
    const rootId = this.sessionStore.getRoot(sessionId);
    const history = this.messageStore.getHistory(rootId);
    for (const message of history) {
      const meta = peerAskMetadata(message);
      if (!meta) continue;
      return (
        meta.fromAgentId === callerAgentId && meta.toAgentId === targetAgentId
      );
    }
    return false;
  }

  private buildPeerAskText(request: AgentAskRequest): string {
    return (
      `<peer-request from="@${request.callerAgentId}" ` +
      `source_session="${request.callerSessionId}">\n` +
      `${request.message}\n` +
      `</peer-request>`
    );
  }

  private async persistPeerAskFailure(
    sessionId: string,
    targetAgentId: string,
    error: unknown,
    ...
    const message =
      (error instanceof Error ? error.message : String(error)).slice(
        0,
        PEER_ASK_ERROR_MAX_CHARS,
      ) || "unknown error";
    const assistant: UIMessage = {
      id: randomUUID(),
      role: "assistant",
      parts: [{ type: "text", text: `agent_ask failed: ${message}` }],
    };
    try {
      ...
      log.warn(
        { err: e, sessionId, targetAgentId },
        "agent_ask: failed to persist failure message",
      );
    }
    return {
      ok: false,
      ...
      response: extractMessageText(assistant.parts as unknown[]),
      assistant_message: {
        role: "assistant",
        parts: assistant.parts as unknown[],
      },
  ...
  private async askAgent(request: AgentAskRequest): Promise<AgentAskResult> {
    ...
    if (!targetDef) {
      return {
        ok: false,
        error: `Agent ${JSON.stringify(request.targetAgentId)} not found.`,
      };
    }
    const targetModel = this.resolveModel(targetDef);
    if (!targetModel.provider || !targetModel.model) {
      return {
        ok: false,
        target_agent_id: request.targetAgentId,
        error: `Agent ${request.targetAgentId} has no model configured.`,
      };
    }
    if (targetDef.instantMessagesEnabled === false) {
      return {
        ok: false,
        target_agent_id: request.targetAgentId,
        error: `Agent ${request.targetAgentId} does not accept instant messages. Use task_create for durable delegated work.`,
      };
    }

    const newSession = !request.sessionId;
    const session = request.sessionId
      ? this.sessionStore.get(request.sessionId)
      : this.sessionStore.create(request.targetAgentId, {
          title: `@${request.callerAgentId} asked @${request.targetAgentId}`,
        });
    if (!session) {
      return {
        ok: false,
        target_agent_id: request.targetAgentId,
        error: `Session ${JSON.stringify(request.sessionId)} not found.`,
      };
    }
    if (session.agentId !== request.targetAgentId) {
      return {
        ok: false,
        target_agent_id: request.targetAgentId,
        session_id: session.id,
        error: `Session ${session.id} belongs to agent ${session.agentId}, not ${request.targetAgentId}.`,
      };
    }
    if (
      request.sessionId &&
      !this.ownsPeerAskSession(
        session.id,
      ...
      return {
        ...
        session_id: session.id,
        error:
          "Cannot continue that session: it was not created by a previous agent_ask call from this agent.",
      };
    }
    if (
      this.peerAskRunning.has(session.id) ||
      this.dispatcher.isRunning(session.id)
    ) {
      return {
        ok: false,
        target_agent_id: request.targetAgentId,
        session_id: session.id,
        error: `Session ${session.id} already has a running turn.`,
      };
    }
    ...
    const userMessage: UIMessage = {
      id: randomUUID(),
      role: "user",
      parts: [{ type: "text", text: this.buildPeerAskText(request) }],
      metadata: {
        kind: "agent_chat",
        fromAgentId: request.callerAgentId,
        toAgentId: request.targetAgentId,
        sourceSessionId: request.callerSessionId,
      } satisfies MessageMetadata,
    };

    try {
      this.messageStore.append(session.id, {
        id: userMessage.id,
        role: "user",
      ...
      assistant = {
        ...
        parts: ensureStepBoundaries(
          finalizeOrphanToolParts(assistant.parts as UIMessage["parts"]),
        ),
      };
      if (capturedError && assistant.parts.length === 0) {
        assistant.parts = [
          {
            type: "text",
            text: `agent_ask failed: ${extractMessageText([
              { type: "text", text: String(capturedError) },
            ])}`,
          },
        ];
      }
      ...
      const turnHistory = [...history, assistant];
      try {
        agent.fireExtractor({
          sessionId: session.id,
        ...
        log.warn(
          { err: e, agentId: request.targetAgentId },
          "agent_ask: memory.extractor launch failed",
        );
      }
      try {
        agent.fireTitle({
