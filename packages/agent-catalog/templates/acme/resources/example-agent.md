...
- **`agentAskEnabled`** — whether this agent may call `agent_ask` to synchronously ask coworkers for quick answers. Defaults to `true`; set `false` when the agent should only coordinate through durable tasks and comments. Users can change this under Agent → Settings → Agent ask tool.
...
- **`tools`** — environment-touching tools only. System tools (`memory`, `skill_view`, `session_search`, `task_*`, `agent_list`, `ping_user`, `defer_session`) are merged in automatically — do NOT list them here, they'll just be deduped. `agent_ask` is also merged when `agentAskEnabled` is true. Email tools (`email_*`) are added only for agents with a mailbox bound.
...
- **Persona body** — second-person, what _they_ are. Multi-paragraph is fine. Tell them what to do, what to avoid, when to ask.
